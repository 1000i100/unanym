...
if ($vote['contestation_duration'] === 'always') {
    ...
    try {
        ...
        if ($interval->d > 0) {
            $parts[] = $interval->d . ' ' . ($interval->d === 1 ? 'jour' : 'jours');
        }
        if ($interval->h > 0) {
            $parts[] = $interval->h . ' ' . ($interval->h === 1 ? 'heure' : 'heures');
        }
        if ($interval->i > 0) {
        ...
        $data['contestation_left'] = count($parts) > 0
        ...
        $data['contestation_end_human'] = $vote['contestation_duration'] === 'always'
...
$can_contest = $vote['contestation_duration'] !== 'none' && $vote['status'] === 'closed';
...
foreach ($xpath->query('//*[@data-show-if]') as $element) {
    ...
    $match = isset($data[$key]) ? (string)$data[$key] === (string)$value : false;
